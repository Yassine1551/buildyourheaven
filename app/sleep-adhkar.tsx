...
import { sleepAdhkarItems, SleepDhikrItem } from '../services/sleepAdhkar';
...
export default function SleepAdhkarScreen() {
  ...
  const reversedItems = [...sleepAdhkarItems].reverse();
  ...
  const [dalilItem, setDalilItem] = useState<SleepDhikrItem | null>(null);
  ...
  const handleTap = () => {
    ...
    incrementSleepDhikr(activeItem.id);
    ...
    if (newCount >= activeItem.target && !completedItems.has(activeItem.id)) {
      ...
      completeSleepDhikr(activeItem.id);
  ...
  const handleShowDalil = (item: SleepDhikrItem) => {
    setDalilItem(item);
  ...
  const handleShare = useCallback(() => {
    Share.share({
      message: buildAdhkarShareMessage('أذكار النوم', sleepAdhkarItems),
    });
  }, []);
      ...
      <SafeAreaView edges={['top', 'bottom']} style={{ flex: 1 }}>
        ...
        {activeItem ? (
                ...
                return (
                  <View key={item.id} style={styles.stripCard}>
                      ...
                      {item.isQuran ? (
                        <View style={styles.hasanatBadgeRow}>
                          <View style={styles.quranBadgeInline}>
                            <MaterialIcons name="auto-awesome" size={12} color="#A78BFA" />
                    ...
                    <View style={styles.bottomSection}>
                        ...
                        <Animated.View style={[styles.counterWrapper, isCenter ? buttonAnimStyle : undefined]}>
                          <Svg width={RING_SIZE} height={RING_SIZE}>
                            ...
                            <Circle
                              ...
                              cy={RING_SIZE / 2}
                              r={radius}
                              stroke={completedItems.has(item.id) ? '#10B981' : '#A78BFA'}
                              strokeWidth={STROKE_WIDTH}
                      ...
                      >
                        <MaterialIcons name="info-outline" size={16} color="#A78BFA" />
          ...
          <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.listContainer} showsVerticalScrollIndicator={false}>
            {sleepAdhkarItems.map((item, index) => {
        ...
        <View style={styles.dalilModalWrapper}>
          <View style={styles.dalilModal}>
            ...
            <View style={styles.dalilIconCircle}>
              <MaterialIcons name="menu-book" size={28} color="#6366F1" />
            </View>
